...
class TaskHistoryCreate(BaseModel):
    ...
    task_id: uuid.UUID = Field(..., description="Associated task ID")
    ...
    assigned_agent_id: uuid.UUID | None = Field(default=None, description="Assigned agent ID (for assignment events)")
...
class TaskHistoryRead(UUIDSchemaMixin, BaseModel):
    ...
    assigned_agent_id: uuid.UUID | None = Field(default=None, description="Assigned agent ID")
