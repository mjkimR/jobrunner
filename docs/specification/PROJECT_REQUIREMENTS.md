...
- **룰 스크립트 관리:** 사용자가 룰에 매칭되는 실행 스크립트를 직접 개발하여 GitHub 리포지토리에 저장/관리.
- **룰 실행 환경:** FastAPI 기반 백엔드 서버, Docker 컨테이너화, 스케줄 기반 실행 (`pg_cron`).
...
본 시스템의 룰 스케줄링은 DB 레벨에서 처리하여 효율성과 확장성을 확보한다.

- **주요 기술:** Supabase에서 지원하는 `pg_cron` (PostgreSQL Cron 확장)을 메인 스케줄러로 사용한다.
- **실행 흐름:**
    1. **(DB) 예약된 작업 실행:** `pg_cron`이 1분마다 지정된 DB 함수를 호출한다.
    2. **(DB) 실행 대상 룰 조회:** DB 함수는 `rules` 테이블의 `schedule` (cron 표현식)을 파싱하여 현재 시간에 실행되어야 할 룰들을 조회한다.
    3. **(DB) Webhook 호출:** 실행 대상 룰을 찾으면, Supabase의 `http` 확장을 통해 FastAPI 백엔드의 Webhook 엔드포인트(예: `/api/v1/execute-rule/{rule_id}`)를 룰 ID와 함께 호출한다.
    4. **(App) 룰 실행:** FastAPI 백엔드는 Webhook 요청을 받아 해당 `rule_id`의 실행 로직(스크립트 실행, 콜백 처리 등)을 수행한다.
...
| `execution_script_path`| `text` | 실행할 메인 스크립트 경로 | `"scripts/stock/check_price.py"` |
| `on_success_path` | `text` | 성공 시 실행할 콜백 스크립트 | `"callbacks/telegram/send_message.py"` |
| `on_failure_path` | `text` | 실패 시 실행할 콜백 스크립트 | `"callbacks/log/db_log_error.py"` |
| `next_run_at` | `timestampz`| **(예비 필드)** `pg_cron` 미지원 환경을 위한 다음 실행 시간. `nullable`. | `2024-10-26 10:05:00Z` |
...
| ID | 구분 | 설명 |
|---|---|---|
| NFR-01 | 기술 스택 | - **Backend:** FastAPI (Python) <br> - **Database:** Supabase (PostgreSQL) <br> - **Scheduler:** pg_cron <br> - **Storage:** Supabase Storage <br> - **Rule Generation:** High-performance LLM (e.g., Gemini) <br> - **Rule Execution:** Cost-efficient or Local LLM <br> - **Data Source (PoC):** `yfinance` |
...
- **Supabase PostgreSQL (DB 레코드):**
    - Supabase PostgreSQL은 `pg_cron` 확장을 통해 데이터베이스 레코드의 자동 삭제를 지원한다.
    - 특정 기간(예: 30일)이 지난 `rule_executions` 테이블의 레코드와 같이 오래된 데이터를 주기적으로 삭제하는 SQL 명령을 `pg_cron`을 사용하여 스케줄링할 수 있다.
    - 이는 Supabase 대시보드의 Cron Jobs 기능을 통해 설정하거나, SQL 편집기에서 `cron.schedule` 함수를 사용하여 직접 SQL 명령으로 설정할 수 있다.
...
- **서비스 간 통신 아키텍처:** 서비스 간 결합도를 낮추고 유연한 확장을 위해 **Pub/Sub (메시지 브로커) 기반의 이벤트 주도 아키텍처**를 채택한다. 각 서비스(프로듀서)는 단순히 이벤트를 발행하고, 다른 서비스(컨슈머)는 이벤트를 구독하여 독립적으로 처리한다.
    - **초기 구현:** Supabase DB 테이블을 메시지 큐처럼 활용하여 Pub/Sub 패턴을 구현.
    - **장기 로드맵:** 시스템 규모에 따라 RabbitMQ, Kafka 등 전용 메시지 브로커 도입을 고려.
...
- **워크플로우 엔진 후보군 상세 비교:**
    - **Dagster / Prefect (DAG 기반 엔진):**
        - **장점:** 데이터 자산 중심(Dagster) 또는 Pythonic한 동적 워크플로우(Prefect) 정의에 강점.
        - **순환 흐름:** 직접적인 순환은 지원하지 않으나, 상위 제어 로직에서 파이프라인 전체를 `for` 루프 등으로 재실행하는 방식으로 구현 가능.
    - **Temporal (Code-as-Workflow 엔진):**
        - **장점:** 워크플로우 자체가 코드이므로 `for`, `while` 등 네이티브 제어문을 통해 **복잡한 순환/재시도 흐름을 매우 자연스럽게 구현** 가능. 장시간 상태 유지에 특화.
        - **단점:** 초기 운영 복잡도가 상대적으로 높음.
- **LangGraph와의 역할 분담:**
...
### 9.1. 관찰 가능성 및 디버깅 (Observability & Debugging)

이벤트 기반의 분산 시스템은 장애 발생 시 원인 추적이 복잡하므로, 시스템 전반의 가시성을 확보하는 것이 중요하다.

-   **분산 추적 (Distributed Tracing):** 모든 요청 및 이벤트에 `trace_id`를 할당하고, 이 ID를 모든 하위 이벤트, API 호출, 로그 메시지에 일관되게 전파하여 시스템 전반의 흐름을 추적할 수 있도록 한다.
-   **중앙화된 로깅 (Centralized Logging):** 모든 컴포넌트(FastAPI, 워크플로우 엔진, 에이전트 등)에서 발생하는 로그를 중앙 로그 시스템(예: ELK Stack, Graylog)으로 집계하여 관리한다. `trace_id`를 활용한 필터링 및 검색 기능을 통해 디버깅 효율성을 높인다.
-   **워크플로우 엔진 UI 활용:** 도입할 워크플로우 엔진(Dagster, Prefect 등)이 제공하는 UI를 적극 활용하여 워크플로우 실행 상태, 태스크별 로그, 자산 현황 등을 모니터링한다.
...
### 9.5. 외부 에이전트 프레임워크 연동 (Integration with External Agent Frameworks)
...
### 9.6. 비용 효율성을 위한 CLI 중심의 비동기 워크플로우

Google AI Pro Plan과 같이 Gemini CLI 사용량에 이점이 있는 요금제를 적극 활용하여, LLM API 호출 비용을 최적화하는 아키텍처를 도입한다. 이는 모든 작업을 실시간 자동 처리하는 대신, **사용자의 개발 워크플로우에 자연스럽게 통합되는 인간 참여형(Human-in-the-Loop) 비동기 모델**을 핵심 전략으로 삼는다.
...
- **기대 효과:**
    - **비용 최적화:** 대부분의 LLM '계획' 단계를 API 비용 없이 처리한다.
    - **안정성 및 통제력 확보:** LLM이 생성한 코드를 실행 전에 사람이 검토함으로써, 의도치 않은 동작을 방지한다.
    - **다용도 활용:** 이 CLI 상호작용 루프는 신규 작업 생성, 실패한 작업의 재처리, 복잡한 승인 프로세스 등에 유연하게 적용된다.
- **고려사항 (처리 경로 분기):
  - ** 작업의 긴급성, 복잡도에 따라 '실시간 자동 처리 경로'와 'CLI 기반 비동기 처리 경로'를 선택할 수 있는 초기 '라우터' 로직을 설계해야 한다.
  - Workflow 생성이 메인이나, 다른 작업들에도 확장 가능하도록 범용적인 설계가 필요하다. 긴급하지는 않으나 면밀한 검토와 엄격한 권한 관리가 요구되는 작업에 특히 유용하다.
    - 예: 코드 에이전트가 생성한 코드를 검토하고(코드리뷰) 커밋 승인하는 프로세스에서의 human-in-the-loop 개입 시나리오
