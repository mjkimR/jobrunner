...
- **핵심 컨셉:**
  - **(Foundation) Prefect 기반 워크플로우:** 모든 인프라는 로컬 Docker에서 구동된다. Prefect 공식 Skill/MCP를 활용하여 Host Agent가 워크플로우를 제어한다.
  ...
  - **(TBD) Dynamic Prefect:** 동적 코드 로딩은 Prefect 네이티브 기능 검토 후 필요시 구현.
...
| **2. Workflow Engine**     | 주기적/정형화된 작업 실행, Flow-Agent 협업 (Prefect)             |
...
| 작업 유형            | 처리 주체               | 흐름                                        |
| -------------------- | ----------------------- | ------------------------------------------- |
| **주기적/정형 작업** | Host Agent → Prefect    | Host Agent가 1회 등록, Prefect가 지속 실행. |
...
| 용어     | 정의                                         | 예시                   |
| -------- | -------------------------------------------- | ---------------------- |
| **Flow** | Prefect에서 실행되는 코드화된 작업 단위      | `daily_stock_alert.py` |
| **Task** | 사용자 또는 에이전트가 수행해야 할 TODO 항목 | "A지역 여행 플랜 작성" |

- **Flow**는 이미 코드로 정의되어 Prefect가 스케줄/트리거하는 안정적인 실행 단위.
...
### 2.6. Prefect MCP 활용 가능성

> [!NOTE]
> Prefect 생태계에서 제공하는 MCP Server 활용을 검토 중.
> Host Agent가 Prefect Deployment/Flow를 직접 쿼리하고 트리거할 수 있는 가능성 있음.
> (상세 스펙 추후 확인)

---

## 3. Foundation - Prefect 기반 워크플로우

> [!NOTE]
> Prefect 공식 Skill/MCP가 풍부하게 지원되므로, 일반적인 Prefect 사용은 공식 도구로 충분히 커버된다.
> JobRunner는 도메인 특화 Skill과 추가 모듈 연동에 집중한다.

**구성 요소:**

- **로컬 런타임 환경:** 사용자 PC(Local)에서 `Docker Compose`로 Prefect, PostgreSQL, Redis를 구동.
- **Host Agent 연동:** Gemini CLI, Claude Code 등 Skill/MCP 지원 Agent Client가 시스템을 제어.
- **공식 Prefect Skill 활용:**
  - `prefect-expert`: 프로젝트 생성, asset 패턴, 자동화, CLI 커맨드
  - `prefect-integrations`: 82+ 통합 카탈로그
- **JobRunner 도메인 Skill (`jr`):** 본 프로젝트 구성에 특화된 CLI 및 가이드.
  - jr 사용법은 SKILL.md로 Host Agent에 주입됨.
- **Prefect 연동:** Prefect MCP 또는 jr CLI를 통해 제어.
...
- 가능: 상태 조회, 간단한 질문 응답, 기존 Flow 즉시 실행
- 불가 (Host Agent 필요): 새로운 Flow 코드 생성, 복잡한 의사결정, 시스템 설정 변경
...
- **Hybrid 전환:** `prefect-worker` 컨테이너를 라즈베리파이 또는 VPS로 이동.
- **Cloud Native:** 팀 단위 협업/대규모 처리 시 K8s 확장 (현재 고려 대상 아님).

---

## 6. TBD - Dynamic Prefect

> [!NOTE]
> Prefect 네이티브 기능으로 동적 로딩이 가능할 수 있음.
> 실제 구현 필요성 및 방식은 추후 검토.

**검토 항목:**

- Prefect의 `Deployments` 동적 갱신 가능 여부
- `prefect-workers` 활용 가능성
- Flow Storage 런타임 리로드 메커니즘

**잠재적 기능:**

- 동적 Flow 로딩: LLM이 생성한 Python 코드를 즉시 Flow로 등록/실행
- 코드 버전 관리: 생성된 코드의 히스토리 관리 및 롤백 지원
- 향상된 템플릿 시스템: 다양한 유스케이스별 Prefect 코드 템플릿 확장
...
| FR-03 | Prefect | **자동 리로드**  | 파일 변경 감지                   | Must     |
| FR-04 | Prefect | **실행 및 로깅** | 스케줄 실행, 로그 저장           | Must     |
...
graph LR
    ...
    HostAgent -- "공식 Skill/MCP" --> Prefect[Prefect]
    HostAgent -- "도메인 Skill" --> jrCLI[jr CLI]
    jrCLI --> Prefect
    Prefect -- "Execute" --> Worker[Local Worker]
...
│   │   └── pyproject.toml
│   │
│   ├── /workflow               # jr-workflow: Prefect 워크플로우
│   │   ├── /jobs               # LLM 생성 코드 저장
│   │   ├── /templates          # Prefect 코드 템플릿
│   │   ├── prefect.yaml
│   │   └── pyproject.toml
│   │
...
- **환경 분리:** (Phase 1) Agent Worker와 Prefect Execution 분리.
...
| **Orchestration**       | Prefect + 공식 MCP      | Prefect + MCP       |
